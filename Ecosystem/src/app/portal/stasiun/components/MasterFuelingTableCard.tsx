...
interface MasterFuelingTableCardProps {
  initialRecords?: MasterFuelingRecord[];
}

export default function MasterFuelingTableCard({ initialRecords = [] }: MasterFuelingTableCardProps) {
  const [records] = useState<MasterFuelingRecord[]>(initialRecords);
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-slate-800/80 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-700/50">
                <th className="p-4 font-medium whitespace-nowrap shrink-0 align-middle">Q.No</th>
                <th className="p-4 font-medium whitespace-nowrap shrink-0 align-middle">Customer &amp; Unit</th>
                <th className="p-4 font-medium whitespace-nowrap shrink-0 align-middle">Time</th>
                <th className="p-4 font-medium whitespace-nowrap shrink-0 align-middle">Pressure (Bar)</th>
                <th className="p-4 font-medium whitespace-nowrap shrink-0 align-middle">Volume (Nm³)</th>
                <th className="p-4 font-medium whitespace-nowrap shrink-0 align-middle">ATEX Insp.</th>
                <th className="p-4 font-medium text-right whitespace-nowrap shrink-0 align-middle">Status</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-slate-800/50">
              {filteredRecords.map((record) => {
                const isOverpressure = (record.pressure_full_bar ?? 0) > 250;
                const inspections = record.inspections ?? [];
                const hasPreFill = inspections.find(i => i.type === 'PRE_FILL');
                const hasPostFill = inspections.find(i => i.type === 'POST_FILL');
                ...
                return (
                  <tr key={record.id} className="hover:bg-slate-800/30 transition-colors group/row">
                    <td className="p-4 whitespace-nowrap shrink-0 align-middle">
                      <span className="text-sm font-semibold text-slate-300">#{String(record.queue_no).padStart(3, '0')}</span>
                    </td>
                    <td className="p-4 whitespace-nowrap shrink-0 align-middle">
                      <div className="font-medium text-white text-sm">{record.customer_name}</div>
                      <div className="text-xs text-indigo-400 mt-0.5">{record.tube_trailer_no} &bull; {record.lwc} LWC</div>
                    </td>
                    <td className="p-4 whitespace-nowrap shrink-0 align-middle">
                    ...
                    <td className="p-4 whitespace-nowrap shrink-0 align-middle">
                      <div className="flex flex-col gap-1">
                        ...
                        <span className="text-sm font-medium text-white flex items-center gap-1">
                          ...
                          {isOverpressure && <AlertTriangle size={14} className="text-rose-500 shrink-0" />}
                        </span>
                      </div>
                    </td>
                    <td className="p-4 whitespace-nowrap shrink-0 align-middle">
                ...
                <tr>
                  <td colSpan={7} className="p-12 text-center text-slate-500">
                    <div className="flex flex-col items-center gap-2">
                      ...
                      <p className="text-sm font-medium">No fueling records found</p>
                    </div>
                  </td>
                </tr>
              )}
