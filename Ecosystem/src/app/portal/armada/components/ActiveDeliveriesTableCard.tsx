...
import {
  Truck, Search, Plus, MapPin, Clock, FileCheck,
...
interface Props {
  cnfData: RawCNF[];
  horecaData: RawHoreca[];
}

const CNF_STATUS_STYLE: Record<string, string> = {
  Dispatched: 'bg-slate-500/10 text-slate-400 border-slate-500/20',
  Discharging: 'bg-cyan-500/10 text-cyan-400 border-cyan-500/20',
  Returning: 'bg-amber-500/10 text-amber-400 border-amber-500/20',
  Completed: 'bg-emerald-500/10 text-emerald-400 border-emerald-500/20',
  Cancelled: 'bg-rose-500/10 text-rose-400 border-rose-500/20',
};

const HORECA_STATUS_STYLE: Record<string, string> = {
...
export default function ActiveDeliveriesTableCard({ cnfData, horecaData }: Props) {
  const [activeTab, setActiveTab] = useState<'CNF' | 'Horeca'>('CNF');
  const [searchTerm, setSearchTerm] = useState('');
  const [isDispatchModalOpen, setIsDispatchModalOpen] = useState(false);
  const [isGPSModalOpen, setIsGPSModalOpen] = useState(false);
  const [selectedTruck, setSelectedTruck] = useState<{ plat: string; driver: string; type: 'CNF' | 'Horeca' } | null>(null);
  ...
  return (
    <div className="col-span-full rounded-2xl border border-white/10 bg-slate-900/60 backdrop-blur-md p-6 shadow-xl flex flex-col h-full relative overflow-hidden group">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6 gap-4 relative z-10">
        ...
        <div className="flex items-center gap-3 w-full sm:w-auto">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 bg-cyan-500 hover:bg-cyan-600 text-slate-900 rounded-xl text-sm font-bold transition-all shadow-[0_0_20px_rgba(6,182,212,0.3)] whitespace-nowrap shrink-0"
          >
            <Plus size={18} />
      ...
      <div className="overflow-x-auto overflow-y-auto h-[350px] custom-scrollbar rounded-xl border border-slate-800/60 bg-slate-900/40 relative z-10">
        {activeTab === 'CNF' ? (
          filteredCNF.length === 0 && cnfData.length === 0 ? (
            <div className="py-16 text-center text-slate-500 text-sm font-medium">No Surat Jalan CNF records found.</div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 z-20">
                <tr className="bg-slate-900 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-700/50 shadow-sm">
                  <th className="p-4 font-medium whitespace-nowrap">Surat Jalan</th>
                  <th className="p-4 font-medium whitespace-nowrap">Destination</th>
                  <th className="p-4 font-medium whitespace-nowrap">Fleet Unit</th>
                  <th className="p-4 font-medium whitespace-nowrap">Status &amp; Time</th>
                  <th className="p-4 font-medium whitespace-nowrap">Validation</th>
                  ...
                  <tr key={sj.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="p-4">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5 flex items-center gap-1">
                        <Clock size={10} />
                        {sj.depart_time ? new Date(sj.depart_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '-'}
                      </div>
                    </td>
                    <td className="p-4">
                      <div className="text-sm text-cyan-300 font-medium">{sj.customer_id ?? sj.customer_address ?? '-'}</div>
                      {sj.customer_address && (
                    ...
                    <td className="p-4">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5">{sj.no_head} &middot; {sj.driver_name ?? sj.driver_id}</div>
                    </td>
                    <td className="p-4 text-sm text-slate-300">
                      {sj.status === 'Discharging' && sj.prs_start_time && (
                        <span>Discharging @ {new Date(sj.prs_start_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}</span>
                      )}
                      {sj.status === 'Dispatched' && <span>En Route to Client</span>}
                      {sj.status === 'Returning' && <span>Returning to MS</span>}
                      <button
                        onClick={() => openGPS(sj.no_head, sj.driver_name ?? sj.driver_id, 'CNF')}
                        className="ml-2 px-2 py-0.5 bg-slate-800 hover:bg-cyan-500/20 hover:text-cyan-400 text-slate-400 rounded text-xs border border-slate-700 transition-colors"
                      >
                    ...
                    <td className="p-4">
                      <div className="flex gap-1">
                        <div title="PPC" className={`w-5 h-5 flex items-center justify-center rounded-full border ${sj.signed_by_ppc ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
                          <FileCheck size={10} />
                        </div>
                        <div title="Driver" className={`w-5 h-5 flex items-center justify-center rounded-full border ${sj.signed_by_driver ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
                          <Truck size={10} />
                        </div>
                        <div title="Customer" className={`w-5 h-5 flex items-center justify-center rounded-full border ${sj.signed_by_customer ? 'bg-emerald-500/20 border-emerald-500/30 text-emerald-400' : 'bg-slate-800 border-slate-700 text-slate-500'}`}>
                          <CheckCircle2 size={10} />
                        </div>
                      </div>
                    </td>
                    <td className="p-4 text-right whitespace-nowrap">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${CNF_STATUS_STYLE[sj.status] ?? 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`}>
          ...
          filteredHoreca.length === 0 && horecaData.length === 0 ? (
            <div className="py-16 text-center text-slate-500 text-sm font-medium">No Delivery Order Horeca records found.</div>
          ) : (
            <table className="w-full text-left border-collapse">
              <thead className="sticky top-0 z-20">
                <tr className="bg-slate-900 text-xs uppercase tracking-wider text-slate-400 border-b border-slate-700/50 shadow-sm">
                  <th className="p-4 font-medium whitespace-nowrap">Delivery Order</th>
                  <th className="p-4 font-medium whitespace-nowrap">Destination</th>
                  <th className="p-4 font-medium whitespace-nowrap">Vehicle</th>
                  <th className="p-4 font-medium whitespace-nowrap">Exchange (Full/Empty)</th>
                  <th className="p-4 font-medium text-right whitespace-nowrap">Status</th>
                </tr>
              </thead>
              <tbody className="divide-y divide-slate-800/50">
                  ...
                  <tr key={do_.id} className="hover:bg-slate-800/30 transition-colors">
                    <td className="p-4">
                      ...
                      <div className="text-xs text-slate-400 mt-0.5">
                        Depart: {do_.depart_time ? new Date(do_.depart_time).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '-'}
                      </div>
                    </td>
                    <td className="p-4 text-sm font-medium text-cyan-300">
                      {do_.customer_name ?? do_.customer_id ?? '-'}
                    </td>
                    <td className="p-4">
                      <div className="text-sm font-bold text-white">{do_.vehicle_plate}</div>
                      <div className="text-xs text-slate-400 mt-0.5">{do_.vehicle_type} &middot; {do_.driver_name ?? do_.driver_id}</div>
                      <button
                        onClick={() => openGPS(do_.vehicle_plate, do_.driver_name ?? do_.driver_id, 'Horeca')}
                        className="mt-1 px-2 py-0.5 bg-slate-800 hover:bg-cyan-500/20 hover:text-cyan-400 text-slate-400 rounded text-xs border border-slate-700 transition-colors"
                      >
                        <MapPin size={10} className="inline mr-1" /> GPS
                      </button>
                    </td>
                    <td className="p-4">
                      <div className="flex items-center gap-3 text-sm">
                        <div className="flex items-center gap-1 text-emerald-400 font-bold bg-emerald-500/10 px-2 py-0.5 rounded border border-emerald-500/20">
                          <Package size={14} /> +{do_.qty_delivered_full ?? 0}
                        </div>
                        <div className="flex items-center gap-1 text-amber-400 font-bold bg-amber-500/10 px-2 py-0.5 rounded border border-amber-500/20">
                          <Package size={14} /> -{do_.qty_returned_empty ?? 0}
                        </div>
                      </div>
                    </td>
                    <td className="p-4 text-right whitespace-nowrap">
                      <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-medium border ${HORECA_STATUS_STYLE[do_.status] ?? 'bg-slate-500/10 text-slate-400 border-slate-500/20'}`}>
