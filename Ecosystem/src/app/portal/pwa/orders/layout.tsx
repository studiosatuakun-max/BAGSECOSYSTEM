...
const tabs = [
  { id: 'home', label: 'Home', href: '/portal/pwa/orders',
    icon: (active: boolean) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 2} strokeLinecap="round" strokeLinejoin="round">
        <path d="M3 12l9-9 9 9M5 10v10a1 1 0 0 0 1 1h3v-6h6v6h3a1 1 0 0 0 1-1V10"/>
      </svg>
    ) },
  { id: 'scan', label: 'Scan', href: '/portal/pwa/dashboard/scan',
    icon: (active: boolean) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 2} strokeLinecap="round" strokeLinejoin="round">
        <path d="M2 7V2h5M17 2h5v5M22 17v5h-5M7 22H2v-5"/><rect x="7" y="7" width="10" height="10" rx="2"/>
      </svg>
    ) },
  { id: 'orders', label: 'Orders', href: '/portal/pwa/orders/budi-1',
    icon: (active: boolean) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 2} strokeLinecap="round" strokeLinejoin="round">
        <path d="M9 11l3 3L22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/>
      </svg>
    ) },
  { id: 'profile', label: 'Profile', href: '/portal/pwa/orders/budi-1',
    icon: (active: boolean) => (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth={active ? 2.5 : 2} strokeLinecap="round" strokeLinejoin="round">
...
export default function OrdersLayout({ children }: { children: ReactNode }) {
  ...
  const getActiveTab = () => {
    if (pathname?.startsWith('/portal/pwa/orders')) return pathname === '/portal/pwa/orders' ? 'home' : 'orders';
